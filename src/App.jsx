...
export default function App() {
  ...
  // Diproses -> Selesai, atau Selesai -> Belum Dicek (tanpa perlu keterangan baru)
  const cycleStatus = async (id) => {
    if (!user || !ADMIN_EMAILS.includes(user.email)) return;
    const current = reports.find((r) => r.id === id);
    if (!current || current.status === 'Belum Dicek') return; // status ini ditangani startProcessing

    const nextStatus = current.status === 'Diproses' ? 'Selesai' : 'Belum Dicek';
    const tanggalDone = nextStatus === 'Selesai' ? new Date().toLocaleDateString('id-ID') : null;
    const keterangan = nextStatus === 'Belum Dicek' ? null : current.keterangan || null;

    try {
      await updateDoc(doc(db, 'reports', String(id)), { status: nextStatus, tanggalDone, keterangan });
      sendToSheet({ action: 'updateStatus', id, status: nextStatus, tanggalDone, keterangan: keterangan || '' });
    } catch (e) {
  ...
  // Belum Dicek -> Diproses: buka kotak input keterangan kecil dulu
  const startProcessing = (r) => {
    if (!user || !ADMIN_EMAILS.includes(user.email)) return;
    if (r.status !== 'Belum Dicek') return;
    setNoteDraftId(r.id);
    setNoteDraftText('');
  };
  ...
  const confirmProcessing = async (id) => {
    try {
      await updateDoc(doc(db, 'reports', String(id)), {
        status: 'Diproses',
        tanggalDone: null,
        keterangan: noteDraftText.trim() || null,
      });
      sendToSheet({ action: 'updateStatus', id, status: 'Diproses', tanggalDone: '', keterangan: noteDraftText.trim() });
    } catch (e) {
      console.error('Gagal mengubah status', e);
      setDbError('Gagal mengubah status. Coba lagi.');
    } finally {
                ...
                {[...reports].reverse().map((r) => (
                  <tr key={r.id} className="border-b border-pink-50">
                    <td className="p-4">
                      ...
                      {r.keterangan && (
                        <div className="text-xs text-yellow-700 bg-yellow-50 border border-yellow-200 rounded-lg px-2 py-1 mt-2 inline-block">
                          Tahap: {r.keterangan}
                        </div>
                      )}
                      {r.tanggalDone && (
                      ...
                      {noteDraftId === r.id && (
                        <div className="mt-3 flex flex-col gap-2 bg-gray-50 border border-pink-200 rounded-xl p-3">
                          <label className="text-xs font-medium text-gray-500">Keterangan tahapan proses (opsional)</label>
                          <input
                            type="text"
                            autoFocus
                            value={noteDraftText}
                            onChange={(e) => setNoteDraftText(e.target.value)}
                            placeholder="misal: Menunggu sparepart datang"
                            className="w-full p-2 text-sm border border-pink-200 rounded-lg"
                          />
                          <div className="flex gap-2">
                            <button
                              onClick={() => confirmProcessing(r.id)}
                              className="px-3 py-1.5 text-xs font-bold rounded-lg bg-yellow-300 hover:bg-yellow-400"
                            >
                              Simpan & Tandai Diproses
                            </button>
                            <button
                              onClick={cancelProcessing}
                              className="px-3 py-1.5 text-xs font-medium rounded-lg border border-gray-300 hover:bg-gray-100"
                            >
                              Batal
                            </button>
                          </div>
                    ...
                    <td className="p-4 align-top">
                      <button
                        onClick={() => (r.status === 'Belum Dicek' ? startProcessing(r) : cycleStatus(r.id))}
